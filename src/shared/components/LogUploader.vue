    ...
    >
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-white/[0.02]">
        <div class="flex items-center gap-3">
          ...
          <span class="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">Ingestor Console</span>
        </div>
      ...
      <div class="relative w-full h-[400px]">
        ...
        <div v-if="!raw" 
           ...
           <h3 class="text-lg font-black text-slate-700 dark:text-slate-200 mb-3 tracking-tight">Sube o arrastra tus logs aquí</h3>
        ...
        <div v-else class="w-full h-full flex flex-col bg-slate-50/30 dark:bg-black/10">
           <div class="flex items-center justify-between px-4 py-2.5 border-b border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-[#131315]">
             <div class="flex items-center gap-2 px-2">
                ...
                <span class="text-[10px] font-black uppercase tracking-[0.15em] text-slate-500 dark:text-slate-400">Datos Originales (Crudo)</span>
             </div>
           </div>
           <textarea
        ...
        <div v-if="store.isProcessing" class="absolute inset-0 flex items-center justify-center bg-white/80 dark:bg-[#0a0a0b]/90 backdrop-blur-md z-10 animate-in fade-in duration-300">
           <div class="flex flex-col items-center gap-5">
              ...
              <span class="text-[11px] font-black uppercase tracking-[0.4em] text-indigo-600 dark:text-indigo-400 animate-pulse">Analizando Trazas...</span>
           </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-6 px-2 mt-6">
      <div class="w-full sm:w-auto">
        <button v-if="raw || leftovers.length > 0" 
          @click="clearEditor"
          class="group flex items-center justify-center gap-2 w-full sm:w-auto px-6 py-3.5 rounded-2xl text-rose-500/70 hover:text-rose-600 dark:hover:text-rose-400 font-black text-[10px] uppercase tracking-widest hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-all active:scale-95">
      ...
      <div class="flex flex-wrap items-center gap-3 w-full sm:w-auto">
        <button 
          ...
          class="group flex items-center justify-center gap-2 flex-1 sm:flex-none px-6 py-3.5 rounded-2xl border-2 border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-[10px] font-black uppercase tracking-widest text-slate-600 dark:text-slate-300 hover:border-indigo-500/50 hover:bg-slate-50 dark:hover:bg-white/10 transition-all active:scale-95 shadow-sm disabled:opacity-50 disabled:active:scale-100"
        >
          <!-- Loader icon -->
          ...
          <span class="w-[90px] inline-block text-center">{{ isReadingFile ? 'Leyendo...' : (isOpeningDialog ? 'Abriendo...' : 'Subir Archivo') }}</span>
        </button>
