...
const handleLogProcess = async (payload: string) => {
  ...
  const promise = () => new Promise((resolve, reject) => {
    try {
      // El store ejecuta el parser y actualiza su estado interno
...
/**
 * Limpia el estado global y local
 */
const handleReset = () => {
  store.clearLogs();
  parseErrors.value = []; 
    ...
    <transition name="fade" mode="out-in">
      ...
      <div v-if="showUploader" key="uploader" class="space-y-10">
        ...
        <div v-if="parseErrors.length > 0" class="animate-in fade-in slide-in-from-top-4 duration-500">
          <div class="flex items-center gap-3 mb-4 text-red-400/80">
          ...
          <div class="bg-red-500/5 border border-red-500/10 rounded-2xl overflow-hidden backdrop-blur-sm">
            <table class="w-full text-left font-mono text-[11px]">
              <thead class="bg-red-500/10 text-red-300/60 uppercase tracking-tighter text-[10px]">
                <tr>
                  <th class="px-6 py-3 border-b border-red-500/10 w-20">Línea</th>
                  <th class="px-6 py-3 border-b border-red-500/10">Descripción</th>
                  <th class="px-6 py-3 border-b border-red-500/10">Contenido Parcial</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-red-500/10">
                <tr v-for="err in parseErrors.slice(0, 10)" :key="err.line" class="hover:bg-red-500/2 transition-colors">
                  <td class="px-6 py-3 text-red-400/80 font-bold">#{{ err.line }}</td>
                  <td class="px-6 py-3 text-red-200/50 italic">{{ err.reason }}</td>
                  <td class="px-6 py-3">
                    <code class="text-red-300/30 bg-red-950/20 px-2 py-0.5 rounded truncate block max-w-md font-mono">
                      {{ err.content }}
                    </code>
                  </td>
                </tr>
            ...
            <div v-if="parseErrors.length > 10" class="p-3 bg-red-500/5 border-t border-red-500/10 text-center text-[10px] text-red-400/40 uppercase tracking-widest font-bold">
