...
<template>
  <div class="space-y-4">
    ...
    <details v-if="details.payload" class="group/json">
      <summary class="flex justify-between items-center px-1 cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden">
        ...
        <button @click.stop.prevent="copyJSON"
      ...
      <pre class="mt-2 p-4 bg-[#0d0d0e] rounded-2xl text-[10px] text-emerald-400/90 overflow-x-auto border border-white/5 shadow-2xl custom-scrollbar font-mono leading-relaxed ring-1 ring-white/5">{{ JSON.stringify(details.payload, null, 2) }}</pre>
    </details>
  </div>
</template>
