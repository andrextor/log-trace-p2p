  ...
  <div class="space-y-5">
    ...
    <div class="flex flex-wrap items-center gap-3 bg-slate-50 dark:bg-white/2 p-3.5 rounded-2xl border border-slate-100 dark:border-white/5 shadow-xs">
      <div v-if="details.provider && details.provider !== 'API_REST'" class="flex flex-col">
        <span class="text-[8px] text-slate-400 font-black uppercase tracking-widest">Provider</span>
      ...
      <div v-if="details.subType" class="flex flex-col">
        <span class="text-[8px] text-slate-400 font-black uppercase tracking-widest">Event Type</span>
      ...
      <div v-if="details.endpoint && details.endpoint !== 'N/A'" class="flex flex-col">
        <span class="text-[8px] text-slate-400 font-black uppercase tracking-widest">Endpoint</span>
    ...
    <div v-if="details.payload" class="flex flex-wrap items-center gap-2 px-1">
      <template v-for="(val, key) in (details.payload as Record<string, unknown>)" :key="key">
        <div v-if="['session_type', 'session_state', 'actual_session_state', 'state_to_update', 'new_state', 'payment_type', 'collect_type'].includes(String(key)) && val"
          ...
          <span class="font-bold text-slate-400 uppercase tracking-widest">{{ String(key).replace(/_/g, " ") }}</span>
    ...
    <div v-if="details.payload" class="relative">
      <div class="flex justify-between items-center mb-2 px-1">
        <div class="flex items-center gap-2">
          ...
          <span class="text-[9px] text-slate-400 font-black uppercase tracking-widest">Payload</span>
        </div>
